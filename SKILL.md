...
25. **All post-auth routing flows through `pickDestination` (or its successor — there must be exactly one routing helper).** New auth entry points MUST call `resolvePostAuthIdentity` + `pickDestination` (`lib/auth/postAuthResolve.ts`) instead of `redirect(safeNext(...))` directly. Reason: DEBT-037 shipped because the routing decision was scattered across `app/auth/callback/route.ts`, `actions/auth.ts:signIn`, `app/page.tsx`, and `app/(org)/dashboard/page.tsx` — each making its own call. A stakeholder-only auth user signing in via the generic magic-link path bypassed the §8 link helper (isolated to invitation-acceptance flow) and was silently routed into the org-creation wizard. Centralizing routing means one function owns "given identity + ?next, where does this user land?" — no bypass surface for future entry points to silently introduce. The "or its successor" hedge protects the *intent* (one source of truth) against drift where the *letter* (call `pickDestination`) is preserved while accidental helper proliferation defeats the principle. Companion rule: `createOrganization` requires `intent: z.literal('wizard_signup')` Zod input — defense-in-depth so any future caller that forgets to route through the helper still fails validation rather than silently spawning an org. See ADR-031.

26. **All app-public URLs go through `getAppUrl()`.** New code that constructs URLs for outgoing emails (notification dispatch, magic-link `emailRedirectTo`, signup/reset redirect targets, invitation accept URLs, share links) or any other server-side outgoing-link context must call `lib/get-app-url.ts:getAppUrl()` — never `process.env.NEXT_PUBLIC_APP_URL` (or `VERCEL_URL` / `VERCEL_ENV`) directly. The helper is the single authority for origin resolution and applies a VERCEL_ENV-aware priority: production allows `NEXT_PUBLIC_APP_URL` custom-domain override; preview/dev always uses `VERCEL_URL` (defeats the localhost-leak class of bugs that bit DEBT-026 twice in Phase F); local falls back to `NEXT_PUBLIC_APP_URL` or `http://localhost:3000`. The helper reads `process.env` directly — a deliberate exception to ARCHITECTURE-saas.md §6 — so tests can stub via `vi.stubEnv` and Vercel-runtime vars don't bloat the Zod startup schema. See ADR-032. Companion: when a fix surfaces a downstream issue (e.g. DEBT-026's correct preview URL exposing the Vercel per-deploy vs branch-alias PKCE cookie split → DEBT-043), register the follow-up as a separate DEBT entry rather than expanding the mini-session's locked scope.

27. **Notification dispatch + outbound email run AFTER the action transaction commits; per-recipient failures isolated and never roll back the domain write.** Drizzle `db.transaction(...)` blocks may contain ONLY DB writes that should be atomic with the action — the domain mutation plus cheap append-only writes like `project_activity` rows. Anything that calls an external HTTPS service (Resend `sendEmail`, future SMS/push providers, third-party webhooks) MUST happen after `tx.commit()`. Wrap each post-commit side effect in its own `try/catch` and capture failures to Sentry with `scope` / `action` / `event_type` tags; do NOT re-throw. Reason: Session 11 Phase 5 wired the dispatcher inside the transaction per the kickoff's then-current rule; Phase F's first stakeholder-invite request hit Vercel 504 Gateway Timeout because each Resend call (the dispatcher loop AND the existing magic-link email) held the tx open for ~1-2s, and the 10-second function limit kicked in before commit. Beyond the timeout, holding a tx during external network calls is bad practice anyway — a transient Resend rate-limit shouldn't roll back a user's stakeholder invite. The carveout was codified in ADR-033 alongside the DEBT-049 hotfix that refactored all 5 wired actions (`sendMessage`, `moveProjectToStage`, `confirmUpload`, `inviteStakeholder`, `createMilestone`). The 5-action invariant becomes the pattern for all future I/O-emitting actions. Note that recipient-list queries also move outside the transaction — read-only lookups don't need to hold the pooled connection. See ADR-033 and DEBT-049.
...
- v3.5 (10 May 2026): **DEBT-037 hotfix lessons baked in.** Hard Rule 25 added — all post-auth routing flows through `pickDestination` (or its successor — there must be exactly one routing helper) so the §8 decision tree has a single source of truth. Triggered by: DEBT-037 production bug (10 May 2026) where stakeholders signing in via the generic magic-link path were silently made owners of brand-new orgs because the routing decision was scattered across 4 sites with the §8 link helper isolated to invitation-acceptance flow. Hotfix shipped as PR #7 commit `0565777`; pre-fix tag `pre-debt-037-hotfix` at `8ac9c0c`. New ADR-031 enshrines three invariants: `createOrganization` callable only from wizard with `intent='wizard_signup'` literal; all post-auth routing through `pickDestination`; auth callback never inserts into organizations or users. Three new DEBT entries: DEBT-040 ("Create your own firm" link UX), DEBT-041 (`/select-context` page), DEBT-042 (`lower(clients.email)` functional index trigger).
- v3.6 (11 May 2026): **DEBT-026 mini-session lessons baked in.** Hard Rule 26 added — all app-public URLs go through `lib/get-app-url.ts:getAppUrl()`, never `process.env.NEXT_PUBLIC_APP_URL` (or `VERCEL_URL` / `VERCEL_ENV`) directly. Resolves DEBT-026, which bit Phase F twice (Sessions 9 and 10) and forced a magic-link → password sign-in pivot during the DEBT-037 hotfix walk. Mini-session shipped as PR #9 commit `aa6899d`; pre-fix tag `pre-debt-026-mini-session` at `d221142`. New ADR-032 enshrines three invariants: single helper for origin construction; VERCEL_ENV-aware priority (production lets `NEXT_PUBLIC_APP_URL` override, preview/dev always uses `VERCEL_URL`, local falls back to `NEXT_PUBLIC_APP_URL` or `http://localhost:3000`); helper reads `process.env` directly so `VERCEL_URL`/`VERCEL_ENV` stay off the Zod schema and tests can stub per-test via `vi.stubEnv`. New DEBT entry: DEBT-043 (PKCE `code_verifier` cookie mismatch between Vercel per-deploy and branch-alias URLs — surfaced by the DEBT-026 fix's correct redirect_to landing on the per-deploy URL while the form cookie was set on the branch alias; recommend folding into S11 kickoff alongside notification-email URL templating). Companion observation added to Rule 26: when a mini-session's fix surfaces a downstream issue, register a separate follow-up DEBT entry rather than expanding the locked mini-session scope.
- v3.7 (13 May 2026): **Session 11 Phase F lesson baked in: post-commit dispatch carveout.** Hard Rule 27 added — `dispatchNotification` and outbound email (`sendEmail`) MUST run AFTER `tx.commit()`, never inside `db.transaction(...)`. DB-only writes (domain mutation + `project_activity` rows via `logActivityTx`) stay inside the tx for atomicity; HTTPS-bound side effects move out. Per-recipient failures captured to Sentry with isolated try/catch and never roll back the domain write. Triggered by: Session 11 Phase F walk against PR #12 surfaced Vercel 504 Gateway Timeout on `inviteStakeholder` because the dispatcher loop + magic-link Resend call held the tx open longer than 10s (Vercel function limit). The pattern was Session 11 plan Hard Rule 6's original "all atomic" stance — Phase F proved it doesn't survive contact with real Resend latency × 2-3 person test orgs. ADR-033 enshrines the carveout; DEBT-049 closed by the hotfix commit (this commit). One-shot cleanup script `scripts/cleanup-debt049-orphans.ts` removed the orphaned `auth.users` row for the test email so retest can proceed. Sessions 6+ wired actions all refactored to the new pattern in one pass (no incremental migration since they all live in `actions/*.ts` and the change is mechanical: move dispatch loop out of tx, wrap each call in try/catch, log to Sentry on failure).