...
| User name inconsistency between header and dashboard | MEDIUM 4 | (closed — not a bug) | The "Hasnath Abdus" string in the top-right header is the **org name** rendered by `OrgSwitcher` (`organizations.name · role`). The "Hasnath Salique" name in the dashboard `Welcome` line and the team members table is the **user name** from `public.users.name`. The org happens to be named after a person, which made the QA tester read it as a user-name discrepancy, but the rendering is internally consistent: `public.users.name` is shown wherever a user is rendered, `organizations.name` wherever an org is rendered (including the team-page intro copy "Anyone here can sign in and use Hasnath Abdus" — which uses `orgName` as a noun). Code audit confirmed zero references to `auth.user_metadata.full_name`. **Optional UX follow-up (not in this hotfix):** add a building/firm icon or "Workspace:" prefix in the OrgSwitcher trigger so the org-name vs user-name distinction is visually obvious without context. |

### Browser verification — production QA replay (2026-05-06, deploy `276e436`)

Verified on `https://pcd-saas.vercel.app` signed in as `contact@plancraftdaily.co.uk` (org "Hasnath Abdus", role Owner). All 9 steps pass.

| # | Step | Fix verified | Result |
|---|---|---|---|
| 1 | Dashboard loads, header shows org name + role only | MEDIUM 4 visual | **PASS** — header: "Hasnath Abdus · Owner" (org+role); body: "Welcome, Hasnath Salique" (user name); footer: "Signed in as contact@plancraftdaily.co.uk · owner" (email+role). All three sources are different and correct. |
| 2 | OrgSwitcher single-org variant has no hover state | MEDIUM 3 | **PASS** — hovering produces no visual change, no cursor-pointer. |
| 3 | Click OrgSwitcher → no dropdown | MEDIUM 3 | **PASS** — click does nothing; page state unchanged. |
| 4 | /settings/team members table shows own row with `(you)`, no kebab | BLOCKER 2 UI | **PASS** — single row "Hasnath Salique (you)" / contact@plancraftdaily.co.uk / Owner; **kebab cell empty on own row**. |
| 5 | Send invitation to `qa-test-1778016500@example.com` → no error boundary, warning toast | BLOCKER 1 | **PASS** — yellow warning toast: "Invitation created for qa-test-1778016500@example.com, but the email could not be delivered. They'll need to be sent the link another way." No error boundary; page intact. |
| 6 | Pending invitations list refreshes with new row | BLOCKER 1 + revalidate | **PASS** — within ~2s the empty-state turned into "1 invitation awaiting acceptance" with the test email row. No manual reload. |
| 7 | Click kebab on pending row → confirm dialog appears | UI flow | **PASS** — dropdown showed "Cancel invitation" (red destructive item); clicking opened the dialog "Cancel invitation to qa-test-1778016500@example.com?" with Keep / Cancel buttons. |
| 8 | Click "Cancel invitation" → row disappears without manual reload | MEDIUM 5 | **PASS** — button showed "Cancelling…" briefly, dialog auto-closed, list reverted to "No pending invitations.", green success toast "Invitation to qa-test-1778016500@example.com cancelled." All without page reload. |
| 9 | Walk dashboard / settings / team noting user-name display | MEDIUM 4 final | **PASS** — every user-name location renders `public.users.name` ("Hasnath Salique"); every org-name location renders `organizations.name` ("Hasnath Abdus"). Internally consistent. MEDIUM 4 closed as not-a-bug. |

**Cleanup:** the test invitation `qa-test-1778016500@example.com` was cancelled in step 8 (deleted_at set; row hidden). No manual cleanup needed. No test projects created.

**Sentry post-deploy:** zero unresolved issues in the last hour at the `276e436` release tag (the BLOCKER 1 trace `c2cceb69` from the previous deploy did not recur after the fix). The screenshots from the QA replay are MCP-captured (IDs in conversation log: `ss_731745fue` dashboard, `ss_7485wjabv` /settings/team initial, `ss_3261f6js3` warning toast on send, `ss_98748uyab` pending list refresh, `ss_1269xwc9g` cancel dialog, `ss_9582ix0k7` post-cancel empty state, `ss_24374azbv` /settings) — they were not persisted to disk because the MCP `save_to_disk` parameter didn't surface a path; future hotfix sessions should plumb a stable screenshot-export path or use `Write` to dump base64 directly.
