// Phase 2 Session 13 — invoice-sent client email. Plain HTML; React Email
// migration is Phase 5 polish (matches quote-sent.ts / team-invitation.ts
// pattern). Subject + body are rendered via the email helper from
// actions/documents.ts on the post-commit path per ADR-033 / Hard Rule 27.
...
export function invoiceSentEmail(input: {
  ...
  const subtypeLabel = input.subtype === 'final' ? 'final' : 'initial';
...
<html lang="en">
  <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; line-height: 1.5; color: #111; max-width: 560px; margin: 0 auto; padding: 24px;">
    <p>Hi,</p>
    <p>Your ${subtypeLabel} invoice <strong>${invoiceNumber}</strong> is ready to view.</p>
    <p>Total: <strong>${total}</strong></p>
    <p style="margin: 24px 0;">
      <a href="${viewUrl}" style="background: #111; color: #fff; padding: 10px 18px; border-radius: 8px; text-decoration: none; display: inline-block;">View invoice</a>
    </p>
